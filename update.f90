...
if(input.eq.1) then
   ...
   write(*,*) "input=1 exp_mhalf_K; input=2 exp_K"
   call mystop
end if
...
!---------------------------------
!update exp_V to the wave function
!---------------------------------
subroutine V_phi(i,ovlp,bond_label)
use param
use mc_loop_param
use phi_param
use lattice_param
...
implicit none
integer,intent(IN)::i
character,intent(IN)::bond_label
complex(kind=8),intent(INOUT)::ovlp(Ntot,Ntot,Dtot)
...
if(bgset.eq.0) then !use mean field
   ...
   call modGS_i(i)
   do bond_ind=1,Nbonds,1
      ! map bond l --> sites q, r
      q=bond_to_sites(1,bond_ind)
      r=bond_to_sites(2,bond_ind)
      if(dtype.EQ.'c') then
         avgnqu=zero
         avgnru=zero
         do k=1,Ntot,1
            avgnqu=avgnqu+(abs(phi(q,k,i)))**2
            avgnru=avgnru+(abs(phi(r,k,i)))**2
         end do
         avgnqd=zero
         avgnrd=zero
         do k=1,Ntot,1
            avgnqd=avgnqd+(abs(phi(q+Nsite,k,i)))**2
            avgnrd=avgnrd+(abs(phi(r+Nsite,k,i)))**2
         end do
      else if(dtype.EQ.'d') then
         avgnqu=zero
         avgnru=zero
...
if(Nbands.eq.1)then
   ...
   do l=1,Nbonds,1
      call single_V_phi(i,l,ovlp(1,1,1),Vnn,nn_decomp,Nbonds,bond_to_sites)
      if(weight(i).le.0.d0) exit
   end do
...
!------------------------------------
!update exp_V_{j} to the wave function i
!with j denoting bond index
!------------------------------------
subroutine single_V_phi(i,j,inver,Vnn,nn_decomp,Nbonds,bond_to_sites)
use param
use rand_num
use phi_param
use model_param
...
if(nn_decomp.eq.1) then !Use continous spin decomposition
   ...
   lr_dn=-x*sqrt(dcmplx(dt*Vnn))
   lr_dn=lr_dn-(dt*Vnn/2.d0)-dt*Vnn*ng(j)

   tot_weig=-1.d0*x*sqrt(dcmplx(dt*Vnn))*ng(j)
   tot_weig=tot_weig-0.5d0*dt*Vnn*(ng(j)**2)
   ...
   lr_dn=x*sqrt(dcmplx(-1.d0*dt*Vnn))
   lr_dn=lr_dn+(dt*Vnn/2.d0)*(1.d0-2.d0*ng(j))

   tot_weig=-1.d0*x*sqrt(dcmplx(-1.d0*dt*Vnn))*ng(j)
   tot_weig=tot_weig+0.5d0*dt*Vnn*(ng(j)**2)
...
!get the new determinant
 if(dtype.EQ.'c') then
   ...
   do k=1,Nspin(1),1
      phi(q,k,i)=phi(q,k,i)*(explr_up+one)
      phi(s,k,i)=phi(s,k,i)*(explr_up+one)
   end do
   do k=Nspin(1)+1,Ntot,1
      phi(q+Nsite,k,i)=phi(q+Nsite,k,i)*(explr_dn+one)
      phi(s+Nsite,k,i)=phi(s+Nsite,k,i)*(explr_dn+one)
   end do
 end if
