...
if(PP.EQ.0) then   !Read wave function from file
  !read Nw 
  open(unit=10,file='phi_N.dat',status='old')
  read(10,*) Nw
  close(10)
else if (PP.EQ.1) then   !Set phi from the trial wave function
  Nw=Dtot
else
...
!Nw = (if reading from file) number of Slater Determinants in initial wf
#ifdef MPI
